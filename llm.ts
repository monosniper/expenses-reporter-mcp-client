...
class LLM {
	...
	constructor() {
		...
		this.instructions = `
			...
			Если пользователь просит дать отчет, ты должен создать его (если еще не было), затем запросить его по id, только не формируй в ответе ссылки на скачивание и кнопки 
